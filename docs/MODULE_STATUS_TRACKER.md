...
| **notifications**              | 🟩 | 🟩 | 🟩 | ⬜ | 🟩 | V11 `notification_outbox` (transactional outbox, one row per recipient, payload = IDs only per Hard Rule #4). `NotificationPublisher` writes rows in the caller's tx; wired into booking (REQUESTED/CONFIRMED/CANCELLED), discussion (NEW_MESSAGE), treatment notes (NOTE_ADDED). V13 `fcm_tokens` + `FcmToken`/`FcmTokenService` + `POST /auth/fcm_tokens` (idempotent upsert keyed on token; controller in `notifications/web` to keep `auth` dependency-free). **Dispatch side now live**: `OutboxDispatcher` (SKIP-LOCKED due query, resolves account→live tokens, retires invalid ones, marks SENT / reschedules with exp backoff / DEAD at `maxAttempts`) driven by `OutboxPoller` (`@Scheduled`, `healyn.notifications.poll-interval-ms`, disabled in tests). Delivery is behind `FcmSenderPort`: `FirebaseFcmSender` (firebase-admin) activates when `HEALYN_FCM_CREDENTIALS_PATH` is set, else `LoggingFcmSender`. Tests: `NotificationPublisherTest`, `FcmTokenServiceTest`, `FcmTokenIntegrationTest`, `OutboxDispatcherTest`, `OutboxDispatcherIntegrationTest`, `FirebaseFcmSenderTest` green. **Remaining**: notification-preferences endpoints (API_STANDARDS §9.8) not yet built; mobile FCM wiring. |
