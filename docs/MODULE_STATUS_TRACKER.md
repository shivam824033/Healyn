...
| **patients**                   | 🟩 | 🟩 | 🟩 | 🟨 | 🟩 | V4 migration; primary patient auto-created at registration; `PatientAccessPolicy` exposed for other modules. **Mobile (Flutter):** patient app shell — 4-tab bottom nav (Home/Appointments/Family/Profile via `StatefulShellRoute.indexedStack`, per-tab back stacks per UI_UX §8.1) + read-only **Profile** (the primary patient from `GET /patients`: identity, DOB/age, sex, contact, blood group, allergies, plus signed-in devices + sign out) and **Family** list (managed family members with empty state). Home greets by first name and shows an appointments placeholder. `PatientSex`/`LocalDateConverter` moved to `shared/`. `flutter analyze` clean; patient-model + Family widget tests green. **Read-only**: create/edit family, edit profile, and device testing deferred. Not merged. |
