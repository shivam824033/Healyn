...
| **notifications**              | 🟨 | 🟨 | 🟦 | ⬜ | 🟨 | V11 `notification_outbox` (transactional outbox, one row per recipient, payload = IDs only per Hard Rule #4). `NotificationPublisher.enqueueToAccount` / `enqueueToPatientManagers` write rows in the caller's tx; wired into booking (REQUESTED/CONFIRMED/CANCELLED), discussion (NEW_MESSAGE), and treatment notes (NOTE_ADDED). V13 `fcm_tokens` + `FcmToken`/`FcmTokenService` + `POST /auth/fcm_tokens` (idempotent upsert keyed on token, controller in `notifications/web` to keep `auth` dependency-free). `NotificationPublisherTest` + `FcmTokenServiceTest` + `FcmTokenIntegrationTest` green. **Dispatch side (next chunk)**: outbox poller + `FcmSenderPort`/adapter + retry/backoff + token resolution/pruning. Will land behind a port with a logging adapter; the real `firebase-admin` adapter awaits dependency approval (Hard Rule #9). FCM credentials provisioned in dev env. |
