...
- The physiotherapist's Today screen surfaces the same account-wide "Unread" total (across every live thread, not just the selected day); tapping it opens an *index* of unread threads — patient name, appointment number, unread count, last-message preview, date — each opening that thread. *Phase 1 implementation note*: computed with the same client-side fan-out over the physio's live appointments (bounded to one page); the server-side aggregate above remains the scale enabler for a large practice.
