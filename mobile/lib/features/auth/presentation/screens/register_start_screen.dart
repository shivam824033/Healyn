...
class RegisterStartScreen extends ConsumerStatefulWidget {
  const RegisterStartScreen({super.key});

  @override
  ConsumerState<RegisterStartScreen> createState() =>
      _RegisterStartScreenState();
}

class _RegisterStartScreenState extends ConsumerState<RegisterStartScreen> {
  ...
  Future<void> _submit() async {
    ...
    try {
      final challengeId = await ref
          ...
          .startRegistration(target);
      if (!mounted) return;
      unawaited(
        context.push(
          '/register/verify',
          extra: RegisterVerifyArgs(challengeId: challengeId, target: value),
        ),
      );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const HealynAppBar(title: 'Create account'),
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Form(
            ...
            child: Column(
              ...
              children: [
                const Text('Get started', style: HealynTypography.h1),
